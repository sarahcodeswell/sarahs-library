// Final script to regenerate all descriptions and fetch ISBNs
...
async function regenerateAll() {
  ...
  for (let i = 0; i < books.length; i++) {
    ...
    if (description) {
      ...
      if (!updateError) {
        updated++;
        console.log(`  ✓ Updated${isbnData?.isbn ? ' + ISBN' : ''}`);
      } else {
        failed++;
  ...
  console.log(`Success rate: ${Math.round(updated/books.length*100)}%`);
}
