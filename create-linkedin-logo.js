// Create LinkedIn company page logo (300 × 300 pixels, square)
import fs from 'fs';
...
const outputPath = path.join(__dirname, 'public', 'linkedin-logo.png');

// Create square SVG with book stack icon centered
const logoSvg = `
<svg width="300" height="300" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Cream background -->
  <rect width="300" height="300" fill="#FDFBF4"/>
  
  <!-- Book stack icon (scaled and centered for square format) -->
...
sharp(Buffer.from(logoSvg))
  ...
  .toFile(outputPath)
  .then(() => {
    console.log('✓ Successfully created linkedin-logo.png');
    console.log(`  Output: ${outputPath}`);
    console.log('  Dimensions: 300 × 300 pixels (square)');
    console.log('  Background: #FDFBF4 (cream)');
    console.log('  Use: LinkedIn company page logo');
  })
  .catch(err => {
    console.error('Error creating LinkedIn logo:', err);
    process.exit(1);
  });
