...
function CuratorBookModal({ book, isOpen, onClose, userHasBook, onAddToQueue, isLoggedIn }) {
  ...
  // Fetch enriched cover and genres - always call hooks unconditionally
  ...
  useEffect(() => {
    ...
    if (enrichedDescription || isEnrichingDescription) return;
    ...
    generateBookDescriptions([{ title: book.title, author: book.author }])
      .then((result) => {
        ...
        setEnrichedDescription(result[key] || bookDesc || null);
      })
      .catch(() => {
        setEnrichedDescription(bookDesc || null);
      })
  ...
  const displayCover = enrichment?.coverUrl || book.coverUrl;
  const displayGenres = enrichment?.genres || [];
    ...
    >
      ...
      >
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-60px)] p-4 sm:p-6">
          ...
          ) : (enrichedDescription || book.description) ? (
            <div className="mb-6">
              <h4 className="text-sm font-medium text-[#4A5940] mb-2">About this book</h4>
              <ExpandableDescription text={enrichedDescription || book.description} />
            </div>
          ) : null}
