...
// Expandable description component with inline "read more" link
function ExpandableDescription({ text, className = '' }) {
  ...
  return (
    <div className={className}>
      <p 
        ref={textRef}
        className={`text-xs text-[#5F7252] leading-relaxed ${isExpanded ? '' : 'line-clamp-4'}`}
      >
        {text}
      </p>
      {needsExpansion && !isExpanded && (
        <button 
          ...
          className="text-xs text-[#7A8F6C] hover:text-[#5F7252] font-medium mt-1"
        >
          read more
        </button>
      )}
